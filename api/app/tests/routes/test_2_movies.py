...
# TODO: Add tests for created_at and for user movies endpoint
def test_get_all_movies_order_by(client):
    ...
    for movie, like in data:
        response = client.post(
            "/movies/{}/vote/".format(movie), json.dumps({"likes": like}))
        assert response.status_code == 200
    ...
    response = client.get(
        "/movies/?order_by=likes_count&direction=asc&skip=8&limit=2")
    assert response.status_code == 200
    response_data = response.json()
    ...
    response = client.get(
        "/movies/?order_by=hates_count&direction=asc&skip=8&limit=2")
    assert response.status_code == 200
    response_data = response.json()
